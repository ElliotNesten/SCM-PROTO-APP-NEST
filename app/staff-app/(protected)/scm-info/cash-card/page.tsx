...
export default async function StaffAppCashCardPage() {
  ...
  const section = scmInfoSettings.cashCard;
  const pagePdfs = pdfSettings.sectionPdfs.cashCard.filter((pdf) => pdf.pdfUrl);
  const itemPdfs = section.items.map((_, index) =>
    (pdfSettings.itemPdfs[getSystemScmInfoItemPdfKey("cashCard", index)] ?? []).filter(
      (pdf) => pdf.pdfUrl,
    ),
  );
