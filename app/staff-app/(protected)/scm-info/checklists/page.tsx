...
export default async function StaffAppChecklistsPage() {
  const [scmInfoSettings, pdfSettings] = await Promise.all([
    getSystemScmInfoSettings(),
    getSystemScmInfoPdfSettings(),
  ]);
  const section = scmInfoSettings.checklists;
  const pagePdfs = pdfSettings.sectionPdfs.checklists.filter((pdf) => pdf.pdfUrl);
  const itemPdfs = section.items.map((_, index) =>
    (pdfSettings.itemPdfs[getSystemScmInfoItemPdfKey("checklists", index)] ?? []).filter(
      (pdf) => pdf.pdfUrl,
    ),
  );
